...
var TILE_SIZE = 512;
var SPRITES_WORLD_SIZE = 2500;
var ENEMY_SPRITES_COUNT = 8;
...
function render(){
    ...
    if (keypadState != undefined) {
    	
    	var state= keypadState;
		
		if ( state.axisX != 0 || state.axisY != 0 ) {
			input.active = true;
		
	        input.angle = angle({x:0,y:0}, {x:state.axisX,y:state.axisY});
	        input.distance = distance({x:0,y:0}, {x:state.axisX,y:state.axisY})*25;
...
function receivedControllerEvent(event) {
...
function init(event) {
	...
	if (cordova) {
		cordova.nativeToJsBridgeMode = 3
	}
    initAudio();
    ...
    document.addEventListener( TOUCH_START, onTouchStart );
    document.addEventListener( window.PGMoga.UPDATE_EVENT, receivedControllerEvent );
